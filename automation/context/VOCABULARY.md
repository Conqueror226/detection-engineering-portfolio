...
| `INSUFFICIENT_CONTEXT` | Host/identity is undeclared, or route policy is not active and complete for the decision scope; cannot judge. |
...
| `CRITICAL_UNAPPROVED_PATH` | Reaches a Tier-0/critical asset by a route prohibited by active, complete policy — regardless of entitlement. |
...
Route policy itself is three-state: `AUTHORIZED`, `PROHIBITED`, or
`UNKNOWN_CONTEXT`. A route missing from an allow list is not automatically
prohibited; the policy must declare the relevant environment, identity, and tier
scope complete for the event date.
